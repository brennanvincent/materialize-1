...
impl Coordinator {
    ...
    async fn peek_stage_finish(
        ...
        if ctx.session().vars().emit_timestamp_notice() {
            if let Some(timestamp) = timestamp {
                ctx.session()
                    .add_notice(AdapterNotice::QueryTimestamp { timestamp });
            }
        }
    ...
    pub(super) async fn sequence_subscribe(
        ...
        let as_of = self
            .determine_timestamp(ctx.session(), &id_bundle, &when, cluster_id, timeline, None)?
            .timestamp_context
    ...
    pub(super) fn sequence_explain(
        ...
        match plan.stage {
            ...
            _ => {
                let result = self.sequence_explain_plan(&mut ctx, plan, target_cluster);
                ctx.retire(result);
            }
        }
    }
                ...
                let create_sql_to_stmt_deps =
                    |coord: &Coordinator, session: &Session, create_source_sql| {
                        ...
                        let create_source_stmt = match mz_sql::parse::parse(create_source_sql)
                            ...
                            .into_element()
                        {
                            (Statement::CreateSource(stmt), _) => stmt,
