...
impl<'a> From<&'a OwnedDatum> for Datum<'a> {
    fn from(other: &'a OwnedDatum) -> Self {
        match other {
            OwnedDatum::Datum(d) => *d,
            OwnedDatum::Bytes(b) => Datum::Bytes(&b),
...
impl<'a> Datum<'a> {
    pub fn to_owned(self) -> OwnedDatum {
        match self {
            Datum::Bytes(b) => OwnedDatum::Bytes(b.to_owned()),
            Datum::String(s) => OwnedDatum::String(s.to_owned()),
            Datum::List(l) => OwnedDatum::List(l.data().to_owned()),
            Datum::Dict(d) => OwnedDatum::Dict(d.data().to_owned()),
            self2 => {
                let d = match self2 {
                    Datum::Null => Datum::Null,
                    Datum::False => Datum::False,
                    Datum::True => Datum::True,
                    Datum::Int32(x) => Datum::Int32(x),
                    Datum::Int64(x) => Datum::Int64(x),
                    Datum::Float32(x) => Datum::Float32(x),
                    Datum::Float64(x) => Datum::Float64(x),
                    Datum::Date(x) => Datum::Date(x),
                    Datum::Time(x) => Datum::Time(x),
                    Datum::Timestamp(x) => Datum::Timestamp(x),
                    Datum::TimestampTz(x) => Datum::TimestampTz(x),
                    Datum::Interval(x) => Datum::Interval(x),
                    Datum::Decimal(x) => Datum::Decimal(x),
...
impl<'a> From<AvroScalar> for Datum<'a> {
    fn from(s: AvroScalar) -> Datum<'a> {
        match s {
            ...
            AvroScalar::Timestamp(ts) => Datum::Timestamp(ts),
        }
    }
}
